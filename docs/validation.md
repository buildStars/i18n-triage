...
## `locales` 实测（2026-09-09）

在 vue-element-plus-admin 的副本上跑 `i18n-triage locales .`（默认配置，源语言 zh-CN）：

| 指标               | 数值                                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------------------------- |
| 语言包             | `apps/admin/src/locales/zh-CN.ts`、`en.ts`，各 87 个 key                                                      |
| 完整度             | en 87/87，缺失 0、多余 0、空值 0                                                                              |
| 扫描代码文件       | 84（含 7 个 mock 文件）                                                                                       |
| 静态引用到的 key   | 45                                                                                                            |
| 死 key             | 0                                                                                                             |
| 可能通过字面量引用 | 42：标签页右键菜单 `common.close*` 4 个、布局名 `layout.*` 6 个、`router.*` 8 个、图表标签 `analysis.*` 24 个 |
| 未定义 key         | 0                                                                                                             |
| 动态调用无静态前缀 | 17 处                                                                                                         |

三轮迭代的数字说明了为什么「死 key」必须保守：

1. 只看 `t()` 的静态实参：42 个死 key，全部是误报。
2. 加上「代码里有字符串字面量恰好等于 key」的启发式：31 个死 key——`common.close*`、`layout.*`、`router.login` 被标签页 / 布局配置里的字面量救回。
3. 再把 mock / 测试文件扫进来：0 个死 key——剩下的 31 个全部出现在 `mock/role`（菜单）与 `mock/analysis`（图表的月份 / 星期 / 渠道）的接口数据里，运行时由后端下发后 `t(item.title)`。

代价是 17 处 `t(key)` 这类无静态前缀的动态调用让工具无法证明任何 key 真的死了，所以报告始终带一行提示；「可能通过字面量引用」这一档是给人复核的，既不当作死 key 也不当作已用。
